	...
	var im = 'img/anim/',
		...
		symbols = {
			"stage": {
				...
				cn: {
					dom: [{
						...
						symbolName: 'openpack',
						t: m,
						r: ['50%', '50%', '1000px', '570px', 'auto', 'auto']
					}],
					style: {
						'${Stage}': {
							isStage: true,
			...
			"openpack": {
				...
				cn: {
					dom: [{
						r: [x7, x7, x8, x9, x10, x10],
						id: x11,
						t: g,
						f: [x12, im + g13, x7, x7, x8, x9]
					}],
					style: {
						'${symbolSelector}': {
							r: [_, _, x14, x15]
...
(function($, Edge, compId) {
	...
	Edge.registerEventBinding(compId, function($) {
		//Edge symbol: 'stage'
		(function(symbolName) {
			Symbol.bindTriggerAction(compId, symbolName, "Default Timeline", 3563, function(sym, e) {
				//sym.$("openpack").hide();
			});
			//Edge binding end
		})("stage");
